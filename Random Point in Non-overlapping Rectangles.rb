=begin

Random Point in Non-overlapping Rectangles

Given a list of non-overlapping axis-aligned rectangles rects, write a function pick which randomly and uniformily picks an integer point in the space covered by the rectangles.

Note:

An integer point is a point that has integer coordinates. 
A point on the perimeter of a rectangle is included in the space covered by the rectangles. 
ith rectangle = rects[i] = [x1,y1,x2,y2], where [x1, y1] are the integer coordinates of the bottom-left corner, and [x2, y2] are the integer coordinates of the top-right corner.
length and width of each rectangle does not exceed 2000.
1 <= rects.length <= 100
pick return a point as an array of integer coordinates [p_x, p_y]
pick is called at most 10000 times.
Example 1:

Input: 
["Solution","pick","pick","pick"]
[[[[1,1,5,5]]],[],[],[]]
Output: 
[null,[4,1],[4,1],[3,3]]
Example 2:

Input: 
["Solution","pick","pick","pick","pick","pick"]
[[[[-2,-2,-1,-1],[1,0,3,0]]],[],[],[],[],[]]
Output: 
[null,[-1,-2],[2,0],[-2,-1],[3,0],[-2,-2]]
Explanation of Input Syntax:

The input is two lists: the subroutines called and their arguments. Solution's constructor has one argument, the array of rectangles rects. pick has no arguments. Arguments are always wrapped with a list, even if there aren't any.

 
=end


class Solution

=begin
    :type rects: Integer[][]
=end
    
    attr_accessor :total_area, :cumulative_area
    
    def initialize(rects)
        @total_area = 0;
        # used to store cumulative area and it's points 
        @cumulative_area = {}
        rects.each do |rect|
            x1 = rect[0];
            y1 = rect[1];
            x2 = rect[2];
            y2 = rect[3];
            length = (x2-x1) + 1; # (5, 12) => 12 - 5 => 7 + 1 => 8 as need to include both x1 and x2
            width  = (y2-y1) + 1;
            area = length*width;
            @total_area += area;
            @cumulative_area[@total_area] = rect;
        end
    end


=begin
    :rtype: Integer[]
=end
    def pick()
        # p cumulative_area
        # {4=>[-2, -2, -1, -1], 7=>[1, 0, 3, 0]}
        # now generating any random number b/w our total_area
        random_no = rand(total_area);
        cumulative_area.each do |k , v|
            # now chcking in which cumulative_area that number is lying 
            if random_no < k
                x1 = v[0];
                y1 = v[1];
                x2 = v[2];
                y2 = v[3];
                #now generating any random number b/w x1 & x2 both inclusive range in that rectangle points 
                x = rand(x1..x2)
                #now generating any random number b/w y1 & y2 both inclusive range in that rectangle points 
                y = rand(y1..y2)
                return [x,y]
            end
        end
    end


end

# Your Solution object will be instantiated and called as such:
# obj = Solution.new(rects)
# param_1 = obj.pick()
